...
public class UmiJWT {
     ...
     * @return
     */
    public static String create(TUser user, String secret, Integer amount, StringRedisTemplate redisTemplate) {
        ...
        String jwt = JWT.create()
                .withJWTId(user.getId())        // 编号, 保持与用户ID相同
                .withKeyId(user.getId())        // Redis KEY使用，用户ID开头，Redis过期时间为最终有效的时间
                .withIssuedAt(now)              // 签发时间
                .withIssuer(user.getName())     // 签发人
