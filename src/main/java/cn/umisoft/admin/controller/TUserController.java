...
import cn.umisoft.admin.util.io.UmiFile;
...
public class TUserController extends UmiTController<ITUserService, TUser> {
    @Value("${umisoft.avatar-disk-prefix-path}")
    ...
    public ResponseEntity<?> getAvatar(String avatar) {
        return ResponseEntity.ok(resourceLoader.getResource("file:" + UmiFile.addSeparator(avatarDiskPrefixPath) + avatar));
    }
